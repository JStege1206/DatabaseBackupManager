...
class BackupManager(
    ...
    private fun cleanUpBackups(backupDirectory: File) {
        ...
        for (backup in backups) {
            ...
            if (now - backupTimestamp < TWO_HOURS) {
            ...
            } else if (now - backupTimestamp < THREE_DAYS) { //72 hours
                // Backup is less than 3 days old. Only one backup per hour should be kept.
                ...
                if (hour != lastHour) {
                    ...
                    log.info {
                        "Keep ${backup.name}, " +
                                "hourly backup for backups less than 72 hours old."
                    }
                } else {
            ...
            } else if (now - backupTimestamp < NINETY_DAYS) {
                // Backup is less than 90 days old. Only one backup per day should be kept.
                val day = parseTimeInfoFromFile(backup, TimeInfo.DAY)
                if (day != lastDay) {
                    // There is no backup to be kept for this day yet, thus keep it.
                    lastDay = day
                    log.info {
                        "Keep ${backup.name}, " +
                                "daily backup for backups less than 90 days old."
                    }
                } else {
                    // There already is a backup for this day, this backup can be removed as it is
                    // deemed obsolete.
                    log.info {
                        "Backup ${backup.name} is no longer necessary since there already" +
                                "is a backup for this day."
                    }
                    toRemove += backup
                }
            } else {
                // Backup is older than 90 days, purge it.
                log.info { "Backup ${backup.name} is older than 90 days, marked obsolete." }
                toRemove += backup
            }
        }
